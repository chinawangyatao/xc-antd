...
<GroupedSelect
  ...
  onAddOption={(label, group, color) => createOption(label, group.id, color)}
  onEditOption={(option, group, label, nextGroup, color) => updateOption(option.value, nextGroup.id, label, color)}
/>
```
...
- Pass `colorOptions` to show an Ant Design `ColorPicker` preset panel while creating or editing labels. Only `options[].color` is persisted and rendered; groups never have a color field. The optional color is appended to `onAddOption` and `onEditOption` callbacks.
